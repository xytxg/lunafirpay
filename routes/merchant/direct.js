...
async function ensureDirectPayFeatureEnabled(res) {
  const enabled = (await require('../../utils/systemConfig').getConfig('direct_pay_feature_enabled', '1')) === '1';
  if (!enabled) {
    res.json({ code: -1, msg: '系统已关闭直接收款功能' });
    return false;
  }
  return true;
}
